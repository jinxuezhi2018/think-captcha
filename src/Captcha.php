...
class Captcha
{
    ...
    protected function generate(): array
    {
        ...
        if ($this->config['math']) {
            ...
            if ($this->config['useZh']) {
                $characters = preg_split('/(?<!^)(?!$)/u', $this->config['zhSet']);
            } else {
    ...
    public function create(string $config = null): array
    {
        ...
        $this->config['imageW'] = (int)$this->config['imageW'];
        $this->config['imageH'] = (int)$this->config['imageH'];
        ...
        $fontttf = $ttfPath . $this->config['fontttf'];
        ...
        // 绘验证码
        $text = $this->config['useZh'] ? preg_split('/(?<!^)(?!$)/u', $generator['value']) : str_split($generator['value']); // 验证码
        foreach ($text as $index => $char) {
            $x     = $this->config['fontSize'] * ($index + 1) * ($this->config['math'] ? 1 : 1.5);
            $y     = $this->config['fontSize'] + mt_rand(10, 20);
            $angle = $this->config['math'] ? 0 : mt_rand(-40, 40);
            imagettftext($this->im, (int)$this->config['fontSize'], $angle, (int)$x, (int)$y, $this->color, $fontttf, $char);
        }
        ...
        if ( $this->config['debug'] ) {
            return ['captcha_key'=>$generator['key'],'captcha_code'=>$generator['value'],'src'=>$content,'captcha_time'=>$generator['time']];
        }else{
    ...
    protected function writeCurve(): void
    {
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->im, (int)($px + $i), (int)($py + $i), $this->config['color']); // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->im, (int)($px + $i), (int)($py + $i), $this->config['color']);
                    $i--;
                }
            }
        }
    ...
    protected function writeNoise(): void
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            for ($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($this->im, 5, mt_rand(-10, (int)$this->config['imageW']), mt_rand(-10, (int)$this->config['imageH']), $codeSet[mt_rand(0, 29)], $noiseColor);
            }
        }
    }
    ...
    protected function background(): void
    {
        ...
        @imagecopyresampled($this->im, $bgImage, 0, 0, 0, 0, (int)$this->config['imageW'], (int)$this->config['imageH'], $width, $height);
